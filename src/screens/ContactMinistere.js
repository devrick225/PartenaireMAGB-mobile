...
const ContactMinistere = ({ navigation }) => {
  ...
  const handleDirections = () => {
    const url = Platform.select({
      ios: `maps:0,0?q=${MINISTERE_LOCATION.latitude},${MINISTERE_LOCATION.longitude}`,
      android: `geo:0,0?q=${MINISTERE_LOCATION.latitude},${MINISTERE_LOCATION.longitude}(Ministère de l'Agriculture)`
    });
    ...
    Linking.canOpenURL(url).then(supported => {
      if (supported) {
        ...
        const googleMapsUrl = `https://www.google.com/maps/search/?api=1&query=${MINISTERE_LOCATION.latitude},${MINISTERE_LOCATION.longitude}`;
  ...
  const renderEmailForm = () => (
    <View style={[styles.emailFormContainer, { backgroundColor: dark ? '#2A2A2A' : '#FFFFFF' }]}>
      ...
      >
        ...
        >
          ...
          <View style={styles.headerTextContainer}>
            <Text style={styles.headerTitle}>Contact Ministère</Text>
            <Text style={styles.headerSubtitle}>Agriculture et Équipement Rural</Text>
          </View>

          <View style={styles.headerIcon}>
            <MaterialIcons name="account-balance" size={30} color="#FFD61D" />
          </View>
        </Animated.View>
      </LinearGradient>
...
export default ContactMinistere;